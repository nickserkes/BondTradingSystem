...
public:
  ...
  string to_string() const {
    string sideStr = (side == PricingSide::BID ? "BID" : "OFFER");
    return sideStr + "," + convert_to_fractional(price) + "," + std::to_string(visibleQuantity) + "," + std::to_string(hiddenQuantity);
  }
...
public:
  ...
  string to_string() const {
    return product.GetProductId() + "," + bidOrder.to_string() + "," + offerOrder.to_string();
  }
